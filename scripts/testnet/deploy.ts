...
async function main() {
    ...
    const testnet = true;
    const owner = Address.parse("0QDEhu_wx6a-uvGY7uXmtqrCibuRHx_0VjxMIGJZ_338ce_n");
    const tonstakers = Address.parse("kQATxwZzdboZiwntEm1gIBPWDYDRsfKWM9odt8SBiwM--IvK");
    const jetton_master = Address.parse("kQBLQCzqBQgqmHiaDXM4E5FVguSkkJQ3BvLyK8elqd668RYC");
    ...
    const pool_sinit = await PoolMaster.init(owner, tonstakers, jetton_master, jetton_wallet_code, jetton_wallet_system);;    // Create initial data for our contract
    ...
    const reserve_sinit = await PrizeReserve.init(pool_address, tonstakers, jetton_master, jetton_wallet_code, jetton_wallet_system);;    // Create initial data for our contract
